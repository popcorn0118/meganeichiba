...
if ( ! class_exists( 'BSF_License_Manager' ) ) {
	...
	class BSF_License_Manager {
		...
		public function license_activation_form( $args ) {
			...
			if ( empty( $product_id ) ) {
				esc_html_e( 'Product id is missing.', 'bsf-core' );
			...
			$button_text_activate    = ( isset( $args['button_text_activate'] ) && ! is_null( $args['button_text_activate'] ) ) ? $args['button_text_activate'] : __( 'Activate License', 'bsf-core' );
			$button_text_deactivate  = ( isset( $args['button_text_deactivate'] ) && ! is_null( $args['button_text_deactivate'] ) ) ? $args['button_text_deactivate'] : __( 'Deactivate License', 'bsf-core' );
			...
			if ( false === $is_active || 'false' === $is_active ) {
				...
				if ( true === $bsf_license_allow_email || 'true' === $bsf_license_allow_email ) {
					$popup_license_subtitle = apply_filters( "bsf_license_key_form_inactive_subtitle_{$product_id}", sprintf( '<p>%s</p>', __( 'Click on the button below to activate your license and subscribe to our newsletter.', 'bsf-core' ) ) );
				} else {
					$popup_license_subtitle = apply_filters( "bsf_license_key_form_inactive_subtitle_{$product_id}", sprintf( '<p>%s</p>', __( 'Enter your purchase key and activate automatic updates.', 'bsf-core' ) ) );
				}
			} else {
			...
			if ( true === $is_active || 'true' === $is_active ) {
				...
				$licnse_active_message = __( 'Your license is active.', 'bsf-core' );
