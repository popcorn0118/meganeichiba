	...
	class BSF_Update_Manager {
		...
		public function bsf_add_registration_message( $plugin_data, $response ) {
			...
			if ( ! empty( $bundled ) ) {
				...
				$message = sprintf( __( ' <br>This plugin is came bundled with the <i>%1$s</i>. For receiving updates, you need to register license of <i>%2$s</i> <a href="%3$s">here</a>.', 'bsf-core' ), $parent_name, $parent_name, $registration_page );
			} else {
				/* translators: %1$s: $registration_page %2%s: search term */
				$message = sprintf( ' <i>%s</i>', sprintf( __( 'Please <a href="%1$s">activate your license</a> to update the plugin.', 'bsf-core' ), $registration_page ) );
			}
