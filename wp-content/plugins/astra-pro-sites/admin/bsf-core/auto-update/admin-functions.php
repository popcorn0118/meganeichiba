		...
		if ( 'update-core.php' === $pagenow || 'plugins.php' === $pagenow || 'post-new.php' === $pagenow || 'edit.php' === $pagenow || 'post.php' === $pagenow ) {
			...
			foreach ( $mix as $product ) :
				...
				if ( 'registered' !== $status ) :
					...
					$message = __( 'Please', 'bsf-core' ) . ' <a href="' . esc_url( $url ) . '" class="bsf-core-license-form-btn" plugin-slug="' . esc_html( $product['id'] ) . '">' . __( 'activate', 'bsf-core' ) . '</a> ' . __( 'your copy of the', 'bsf-core' ) . ' <i>' . esc_html( $product['product_name'] ) . '</i> ' . __( 'to get update notifications, access to support features & other resources!', 'bsf-core' );
