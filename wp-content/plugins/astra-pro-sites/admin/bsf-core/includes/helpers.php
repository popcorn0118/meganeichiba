	...
	function bsf_extension_nag( $product_id = '', $mu_updater = false ) {
		...
		if ( '' !== $bsf_not_activated_plugins || '' !== $bsf_not_installed_plugins ) {
			...
			if ( '' !== $bsf_not_activated_plugins ) {
				echo '<p>';
				echo esc_html( $bsf_product_name ) . esc_html__( ' requires following plugins to be active : ', 'bsf-core' );
				echo '<strong><em>';
			...
			if ( '' !== $bsf_not_installed_plugins ) {
				echo '<p>';
				echo esc_html( $bsf_product_name ) . esc_html__( ' requires following plugins to be installed and activated : ', 'bsf-core' );
				echo '<strong><em>';
			...
			if ( true === $bsf_activate ) {
				$installer .= '<a href="' . get_admin_url() . 'plugins.php?plugin_status=inactive">' . __( 'Begin activating plugins', 'bsf-core' ) . '</a> | ';
			}

			if ( true === $bsf_install ) {
				$installer .= '<a href="' . bsf_exension_installer_url( $product_id ) . '">' . __( 'Begin installing plugins', 'bsf-core' ) . '</a> | ';
			}

			$installer .= '<a href="' . esc_url( add_query_arg( 'bsf-dismiss-notice', $product_id ) ) . '">' . __( 'Dismiss This Notice', 'bsf-core' ) . '</a>';
...
if ( ! function_exists( 'bsf_nag_brainstorm_updater_multisite' ) ) {
	...
	function bsf_nag_brainstorm_updater_multisite() {
		...
		sprintf( __( 'Looks like you are on a WordPress Multisite, you will need to install and network activate %1$s Brainstorm Updater for Multisite %2$s plugin. Download it from %3$s here %4$s', 'bsf-core' ), '<strong><em>', '<strong><em>', '<a href="http://bsf.io/bsf-updater-mu" target="_blank">', '</a>' );
